...
func (s *Service) restoreRuntime() {
	...
	if _, err := os.Stat(s.paths.MihomoConfigPath); err == nil {
		...
		s.mu.Unlock()
		if err := s.syncShellProxyEnv(); err != nil {
			s.recordError(fmt.Sprintf("write shell proxy env: %v", err))
		}
		if systemProxyEnabled {
...
func buildShellProxyEnv(running bool, mixedPort int) []byte {
	if running && mixedPort > 0 {
		proxyURL := fmt.Sprintf("http://127.0.0.1:%d", mixedPort)
		return []byte(strings.Join([]string{
			"# generated by hnetd",
			fmt.Sprintf("export HNET_HTTP_PROXY=%q", proxyURL),
			fmt.Sprintf("export HNET_HTTPS_PROXY=%q", proxyURL),
			fmt.Sprintf("export HNET_ALL_PROXY=%q", proxyURL),
			fmt.Sprintf("export http_proxy=%q", proxyURL),
			fmt.Sprintf("export https_proxy=%q", proxyURL),
			fmt.Sprintf("export all_proxy=%q", proxyURL),
			fmt.Sprintf("export HTTP_PROXY=%q", proxyURL),
			fmt.Sprintf("export HTTPS_PROXY=%q", proxyURL),
			fmt.Sprintf("export ALL_PROXY=%q", proxyURL),
...
func (s *Service) syncShellProxyEnv() error {
	s.mu.Lock()
	mixedPort := s.state.MixedPort
	s.mu.Unlock()
	return fileutil.WriteFileAtomic(s.paths.ShellEnvPath, buildShellProxyEnv(s.supervisor.Running(), mixedPort), 0o600)
}
...
func (s *Service) buildStatus(snapshot serviceSnapshot) api.StatusResponse {
	status := api.StatusResponse{
		...
		ShellEnvPath:       s.paths.ShellEnvPath,
...
func (s *Service) handleUnexpectedMihomoExit(err error) {
	...
	if syncErr := s.syncShellProxyEnv(); syncErr != nil {
		s.recordError(fmt.Sprintf("write shell proxy env: %v", syncErr))
	}
